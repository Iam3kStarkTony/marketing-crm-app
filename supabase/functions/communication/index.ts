...
serve(async (req) => {
  ...
  try {
    ...
    console.log('Communication request:', requestData)

    // Get n8n webhook URL and API key from environment
    const webhookUrl = Deno.env.get('N8N_COMMUNICATION_WEBHOOK_URL')
    ...
    if (!response.ok) {
      ...
      return new Response(
        JSON.stringify({ error: 'Failed to trigger communication workflow', details: errorText }),
        { status: 500, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
      )
    }
    ...
    return new Response(
      JSON.stringify({ success: true, message: 'Communication workflow triggered successfully', result }),
